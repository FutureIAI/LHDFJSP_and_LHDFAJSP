...
import numpy as np
import copy
from network import DuelingNet, Net
...
class DQN(object):
    def __init__(self, args):
        ...
        self.target_net = copy.deepcopy(self.net)  # 复制 online_net 到 target_net

        self.optimizer = torch.optim.Adam(self.net.parameters(), lr=self.lr)  # 优化器

    def choose_action(self, state, epsilon):
        with torch.no_grad():
            state = torch.unsqueeze(torch.tensor(state, dtype=torch.float), 0)  # 维度压缩,返回一个tensor，删除input中维度为1的。
            q = self.net(state)
